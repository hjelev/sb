...
mod tests {
    ...
    /// All env vars that influence terminal_image_protocol(). Tests use this set
    /// to start from a known-clean state then override the var(s) under test.
    fn clean_env() -> [(&'static str, Option<&'static str>); 6] {
        [
    ...
    fn override_var(
        base: [(&'static str, Option<&'static str>); 6],
        key: &'static str,
    ...
    fn detects_kitty_from_window_id() {
        let vars = override_var(clean_env(), "KITTY_WINDOW_ID", Some("1"));
        with_vars(vars, || {
            let (proto, _) = terminal_image_protocol();
            assert_eq!(proto, TerminalImageProtocol::Kitty);
        });
    }

    #[test]
    fn detects_kitty_from_ghostty_env() {
        let vars = override_var(clean_env(), "GHOSTTY_RESOURCES_DIR", Some("/opt/ghostty"));
        with_vars(vars, || {
            let (proto, _) = terminal_image_protocol();
            assert_eq!(proto, TerminalImageProtocol::Kitty);
        });
    ...
    fn detects_iterm_from_term_program() {
        let vars = override_var(clean_env(), "TERM_PROGRAM", Some("iTerm.app"));
        with_vars(vars, || {
            let (proto, _) = terminal_image_protocol();
            assert_eq!(proto, TerminalImageProtocol::Iterm2Inline);
        });
    }

    #[test]
    fn detects_sixel_from_wt_session() {
        let vars = override_var(clean_env(), "WT_SESSION", Some("session-id"));
        with_vars(vars, || {
            let (proto, _) = terminal_image_protocol();
            assert_eq!(proto, TerminalImageProtocol::Sixel);
        });
    }

    #[test]
    fn falls_back_to_unsupported_for_plain_xterm() {
        with_vars(clean_env(), || {
            let (proto, _) = terminal_image_protocol();
            assert_eq!(proto, TerminalImageProtocol::Unsupported);
        });
    }
}
